...
/* TODO Put that in database somehow */
const HOLIDAY_EasterRelative = {
 ...
 */
const States = {
  'ch': ['newYear:f', 'ascensionDay:f', 'christmas:f'],
  ...
  'fr_cat': ['nationalCHDay:f', 'goodFriday:f', 'corpusChristi:f', 'assumption:f', 'allSaintsDay:f', 'immaculateConception:f'],
  'fr_pro': ['nationalCHDay:f', 'berchtoldsTag:f', 'goodFriday:f', 'easterMonday:f', 'pentecostMonday:f', 'saintEtienne:f'],
...
function Holiday(year = (new Date()).getFullYear()) {
  if (Holiday._instance) {
    Holiday._instance.addYear(year)
    return Holiday._instance
  }
  this.holidays = {}
  this.rHolidays = {}
  this.addYear(year)
  Holiday._instance = this
}

Holiday.prototype.addYear = function (year) {
  if (this.holidays[year]) { return }
  this.holidays[year] = this.computeYear(year)
...
Holiday.prototype.isHoliday = function (day) {
  ...
  if (days) {
    ...
    return { day: details, c: days, name: I18N.$(details.name) }
  }

  return false
}

Holiday.prototype.getEasterDay = function (year) {
...
Holiday.prototype.computeDay = function (name, year) {
  if (HOLIDAY_EasterRelative[name]) {
    return (() => {
      if (!HOLIDAY_EasterRelative[name]) { return null }
      let rule = HOLIDAY_EasterRelative[name]
      let date = this.getEasterDay(year)
      date.setTime(date.getTime() + (rule * DAY_LENGTH_MS))
      return date
    })()
  }
  if (HOLIDAY_FixedDay[name]) {
    return (() => {
      if (!HOLIDAY_FixedDay[name]) { return null }
      let rule = HOLIDAY_FixedDay[name]
      let [day, month] = rule.split('.', 2)
      return new Date(year, parseInt(month) - 1, day, 12, 0, 0, 0)
    })()
  }
  if (HOLIDAY_RelativeDate[name]) {
    return (() => {
      if (!HOLIDAY_RelativeDate[name]) { return null }
      let rule = HOLIDAY_RelativeDate[name]
...
Holiday.prototype.computeYear = function (year) {
  let days = {}
  let common = []
  States['ch'].forEach((day) => {
    let [name, type] = day.split(':', 2)
    let result = this.computeDay(name, year)
    if (result) {
    ...
    States[c].forEach((day) => {
      ...
      if (name.indexOf('|') !== -1) {
        ...
        for (let i = 0; i < rDays.length; i++) {
          ...
          let r = this.computeDay(rDays[i].substring(0, rDays[i].length - 1), year)
      ...
      if (result === null) {
        result = this.computeDay(name, year)
      }
      if (result) {
