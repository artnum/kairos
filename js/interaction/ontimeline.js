...
function iSelectReservation(event) {
  ...
  const { x, y } = kview.getCurrentBox()
  const cell = kview.getCell(x, y)
    ...
    ui.renderForm()
      .then(domNode => {
        ...
        const ktab = klateral.add(domNode, {
          title: I18N.$('Reservation_x', `${reservation.get('uid')}:${reservation.get('version')}`),
          id: reservation.get('uid'),
          action: [
            { name: 'duplicate-at-end', label: I18N.$('Creer_la_fin') },
            { name: 'delete', label: I18N.$('Supprimer') }
          ]
        })
        if (ktab) {
...
function iZoomCurrentBox(event) {
  ...
  if (kview.getCell(x, y).size < 1) { return }
  const view = document.createElement('DIV')
  view.id = `zoomed_box_${x}_${y}`
  let xpos = kview.getPixelX(x + 1) - 28
  if (xpos + 300 > kview.get('viewport-width')) {
    xpos = kview.getPixelX(x) - 363
  }
  view.style.setProperty('left', `${xpos}px`)

  const entries = kview.getObjectsOnGrid(x, y)
  ...
  for (const [_, entry] of entries) {
    if (top < 0) {
      ...
      top = kview.getRowTop(uinode.rowid)
    }
  ...
  const totalHeight = copies.length * kview.get('entry-height') + 14
  if (totalHeight + top > kview.get('viewport-height')) {
    top -= (copies.length - 1) * kview.get('entry-height')
  } else {
    top -= 14
  }
...
function iGrowAddReservation(event) {
  ...
  const kview = new KView()
  const { x, y } = kview.getCurrentBox()
  const entry = kview.getRowObject(y)
  const travail = new KTaskBar().getCurrentTaskData()
  ...
  const lineX = kview.getXFromDate(begin) * kview.get('day-width') + kview.get('margin-left')
...
function iAddReservation(event) {
  ...
  const begin = new Date()
  if (!kview.get('date-origin')) { throw new Error(I18N.$('ERR:Reload')) }
  begin.setTime(kview.get('date-origin').getTime())
  begin.setTime(begin.getTime() + (x * 86400000))
...
function KIOpenResourceAllocation(day) {
  ...
  Promise.all([
    ...
    .then(([resources, stuffs]) => {
      ...
      popup.add(div, { title: I18N.$('Vehicule_du_x', day.shortDate()) })
